const drawPowerupIcons = (icon) => {
    ...
    let iconPaths = {
        ...
        thick: [pathThick],
        thin: [pathThin],
        // Bachelor powerups: use simple constructed shapes via canvas API below
        shots: [],
        jesus: [],
        circus: [],
    }
    ...
    // ---- Custom canvas-drawn icons for bachelor powerups ----
    if (icon === "shots") {
        // Draw a shot glass
        ctxPV.save()
        ctxPV.fillStyle = yellow
        ...
        ctxPV.beginPath()
        ctxPV.moveTo(-s * 0.45, -s * 0.9)
        ctxPV.lineTo( s * 0.45, -s * 0.9)
        ctxPV.lineTo( s * 0.3,   s * 0.7)
        ctxPV.lineTo(-s * 0.3,   s * 0.7)
        ctxPV.closePath()
        ctxPV.fill()
        // liquid inside (darker fill — shows it's full)
        ctxPV.fillStyle = "#000000"
        ctxPV.beginPath()
        ctxPV.moveTo(-s * 0.35, -s * 0.6)
        ctxPV.lineTo( s * 0.35, -s * 0.6)
        ctxPV.lineTo( s * 0.25,  s * 0.55)
        ctxPV.lineTo(-s * 0.25,  s * 0.55)
        ctxPV.closePath()
        ctxPV.fill()
        // bottom base
        ctxPV.fillStyle = yellow
        ctxPV.fillRect(-s * 0.35, s * 0.7, s * 0.7, s * 0.12)
        ctxPV.restore()
    }
    ...
    if (icon === "jesus") {
        ...
        const arm = s * 0.22
        // vertical bar
        ctxPV.fillRect(-arm * 0.5, -s * 0.9, arm, s * 1.8)
        // horizontal bar (higher up)
        ctxPV.fillRect(-s * 0.8, -s * 0.35, s * 1.6, arm)
        ctxPV.restore()
    }

    if (icon === "circus") {
        // Draw a circus tent: triangular canopy with stripes + a flag on top
        ctxPV.save()
        const s = iconSize * 0.55

        // --- Tent body (full triangle, yellow) ---
        ctxPV.fillStyle = yellow
        ctxPV.beginPath()
        ctxPV.moveTo(0,       -s * 0.75)   // apex
        ctxPV.lineTo( s * 0.9, s * 0.55)   // bottom-right
        ctxPV.lineTo(-s * 0.9, s * 0.55)   // bottom-left
        ctxPV.closePath()
        ctxPV.fill()

        // --- Three dark stripes (vertical cuts through the triangle) ---
        ctxPV.fillStyle = "#000000"
        ctxPV.save()
        // Clip to tent shape so stripes don't bleed outside
        ctxPV.beginPath()
        ctxPV.moveTo(0,       -s * 0.75)
        ctxPV.lineTo( s * 0.9, s * 0.55)
        ctxPV.lineTo(-s * 0.9, s * 0.55)
        ctxPV.closePath()
        ctxPV.clip()
        const stripeW = s * 0.14
        for (const sx of [-s * 0.45, 0, s * 0.45]) {
            ctxPV.fillRect(sx - stripeW / 2, -s * 0.9, stripeW, s * 1.6)
        }
        ctxPV.restore()

        // --- Tent valance (scalloped fringe along the bottom) ---
        ctxPV.fillStyle = yellow
        const scallops = 7
        const scW = (s * 1.8) / scallops
        for (let i = 0; i < scallops; i++) {
            const cx = -s * 0.9 + scW * (i + 0.5)
            ctxPV.beginPath()
            ctxPV.arc(cx, s * 0.55, scW * 0.52, 0, Math.PI)
            ctxPV.fill()
        }

        // --- Flag pole ---
        ctxPV.fillStyle = yellow
        ctxPV.fillRect(-s * 0.04, -s * 1.1, s * 0.08, s * 0.4)

        // --- Triangular flag ---
        ctxPV.fillStyle = "#000000"
        ctxPV.beginPath()
        ctxPV.moveTo(0,        -s * 1.1)
        ctxPV.lineTo( s * 0.3, -s * 0.95)
        ctxPV.lineTo(0,        -s * 0.8)
        ctxPV.closePath()
        ctxPV.fill()
