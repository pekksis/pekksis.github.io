...
let achtung = {
    ...
    powerups: [
        ...
        "r_shots",
        "g_jesus",
		"r_circus"   // <-- ADDED: Circus Tent Sabotage
    ],
...
if (powName == "r_circus") {
    // Spawns a massive obstacle flying across from a random edge
    let side = Math.floor(Math.random() * 4);
    let startX, startY, vx, vy;
    let speedMult = moveSpeed * 1.5;

    if (side == 0) { startX = -40; startY = Math.random() * h; vx = speedMult; vy = (Math.random() - 0.5) * speedMult; } // Left edge
    if (side == 1) { startX = h + 40; startY = Math.random() * h; vx = -speedMult; vy = (Math.random() - 0.5) * speedMult; } // Right edge
    if (side == 2) { startX = Math.random() * h; startY = -40; vx = (Math.random() - 0.5) * speedMult; vy = speedMult; } // Top edge
    if (side == 3) { startX = Math.random() * h; startY = h + 40; vx = (Math.random() - 0.5) * speedMult; vy = -speedMult; } // Bottom edge
